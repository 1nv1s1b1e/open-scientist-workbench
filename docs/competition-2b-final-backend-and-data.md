...
更新日期：2026-08-26。本文记录实际下载、处理和运行得到的结果。它把“闭环是否可复现”“某项预测是否相容”“具体加热机制是否得到区分性支持”分开报告，禁止用流程完整度替代科学结论。
...
7. 每条证据绑定原子预测、样本、事件组、原始数据指纹、观测量族、方法族、分析 split、处理版本、点估计和区间。
...
数值模拟部分目前只做到“文献前向预测进入 RAG、生成 MHD 配置/观测建议、登记 `future:matched-mhd-forward-model` 和 `future:component-ablation-study`”。`mhdConfigTool` 只写 `.cfg` 与建议书，不启动 MHD 求解器、不生成合成 AIA/IRIS/EIS，也不计算候选间似然或留出误差。因此“结合数值模拟寻找判别证据”目前是**部分实现**，不能写成已经完整实现。完整闭环需要新增可版本化的模拟执行器、参数先验、仪器响应卷积、观测/模拟同一特征提取、组件消融和独立事件评分；在这些产物存在之前，规划器应继续把它登记为 `requires_data`，不能为了得到 `supported` 伪造模拟证据。
...
当前提交基线为 `run-1788030664349-d02ffa8c`，结果位于 `output/closure-complete-regression-r3-20260830/scientific-result.json`。该次 local-grounded 冷缓存回归使用 `wcs-roi-dem-v5`，以两路有界并发运行本地处理；20 个任务中 16 个本地任务全部 `completed`（含 5 个逐假设跨事件 holdout 与 AR11899 IRIS），4 个明确依赖新数据/外部设施的任务保持 `planned`。失败任务 0、失败智能体 0、error correction 0。

当前严格结果为 11 条 `uncertain`、0 条 `supported`、0 条 `candidate`、0 条 `eliminated`，总体 `scientificStatus=needs_data`。系统产生 130 条证据，但没有任何记录同时满足机制区分、独立事件、定量区间、全部预测覆盖和 holdout 门槛。把当前结果写成 supported 会违反预注册判据；“预测相容”也不能替代机制证据。

每条假设都有机器可读的主状态和原因状态；当前 11 条均为 `uncertain / deferred_requires_data / insufficient`。`workflowClosure.status=complete` 表示本轮假设和任务均已处置；`operationalClosure.status=complete` 表示智能体、执行器和数据完整性没有失败；`closureStatus=partial` 则诚实表示科学证据仍不足。这三个状态不得合并。
...
下一步数据由具体缺口触发：过程层需要按事先登记选择规则增加独立/更长事件窗和可比较光谱；波动层需要环路径、密度、传播、阻尼和能流闭合；纳耀斑/重联需要事件匹配硬 X 射线与磁拓扑前向模型；耦合层需要匹配观测的 MHD 前向模拟和组件消融。不能在看到结果后只挑“强尾部事件”，也不能承诺新增固定 GB 数后必然 supported。
...
- `output/closure-complete-regression-r3-20260830/scientific-result.json`
- `output/closure-complete-regression-r3-20260830/run-metadata.json`
...
- 自动化测试：最终回归 72 个测试文件无失败；最近一次环境中 576 项通过、11 项 Helix/环境集成测试按设计跳过，并额外通过 PowerShell API-only/动态端口/归档恢复脚本回归。淘汰门禁测试同时覆盖“多事件、holdout 且功效充分时淘汰”和“单事件/功效不足时不得淘汰”；新增测试覆盖 IRIS holdout 绑定、两路有界处理、局部完整性、correction 语义去重、unknown 不制造降级和 operational degraded 路径。
- Monorepo production build：11/11 包通过。Web 已改用离线安全的系统字体栈，构建不再依赖 Google Fonts 网络下载；页面结构未改。
- 运行验收：`run-1788030664349-d02ffa8c` 达到 `workflowClosure=complete`、`operationalClosure=complete`、`closureStatus=partial`；归档 request/manifest/patch SHA 均复算一致，SSE 含且仅含一次 `scientific.loop-complete`。
