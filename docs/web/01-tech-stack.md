...
| 层             | 选型                        | 版本                                                    | 理由                                                                                       |
| -------------- | --------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| 框架           | **Next.js**                 | 16（App Router）                                        | React 19 全路由收敛，Turbopack 开发体验最佳                                                |
| React          | 19                          | —                                                       | Next 16 要求                                                                               |
| Chat UI 框架   | **assistant-ui**            | `@assistant-ui/react@14` + `@assistant-ui/react-ai-sdk` | 生产级 AI chat React 库，与 AI SDK v7 深度集成；per-tool 自定义渲染器                      |
| Transport      | **useRunStream (SSE)**      | 自实现                                                  | 消费 SSE body 解析 UIMessageChunk，tail-relative 断线重连，不需要 Redis                    |
...
| 动画主力       | **Motion**                  | `motion`                                                | 声明式 React 原生，组件 enter/exit + layout 动画 + 手势；30KB                              |
| 3D 知识图谱    | **react-force-graph-3d**    | latest                                                  | ThreeJS/WebGL + d3-force-3d 物理引擎                                                       |
| 演化树         | **d3-hierarchy** + 自写 SVG | latest                                                  | 树布局算法，动画用 Motion                                                                  |
...
|            | Motion                                                                                                                   |
| ---------- | ------------------------------------------------------------------------------------------------------------------------ |
| 范式       | 声明式（`<motion.div animate={...}>`）                                                                                   |
| React 集成 | 原生                                                                                                                     |
| 强项       | 组件 enter/exit、layout 动画、手势、复杂时间线                                                                           |
| 用在哪     | Chat message 滑入、演化树节点生长、approval dialog 弹出、3D 图谱节点 hover、协作大厅多元素同步剧本动画、连线粒子散开特效 |
...
| Redis + resumable-stream    | useRunStream 已自带断线重连（chunk index），不需要额外 Redis      |
| React Spring                | Motion 的 physics spring 够用，避免生态分散                       |
| framer-motion（旧命名）     | 已改名 `motion`，用新包名                                         |
| GSAP                        | Motion 已覆盖全部动画需求（含复杂时间线），避免生态分散           |
