...
function runHyperframes(args) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(
      npxCommand,
      ["--yes", "hyperframes@" + hyperframesVersion, ...args],
      {
        cwd: demoDir,
        env: {
          ...process.env,
...
function buildGateChecks(result, evidenceCounts) {
  ...
  const confidence = Math.max(...(result.hypotheses ?? []).map((item) => Number(item.confidence) || 0), 0);
  return [
    { label: "可量化结果与区间", value: quantitative ? "present" : "missing", state: quantitative ? "pass" : "hold" },
    { label: "支持记录 ≥ 3", value: supportCount + " / 3", state: supportCount >= 3 ? "pass" : "hold" },
    { label: "独立事件与原始谱系", value: "requires ≥ 3", state: "hold" },
    { label: "观测族 / 方法族", value: "requires ≥ 2 / 2", state: "hold" },
    { label: "独立 holdout", value: holdout ? "present" : "missing", state: holdout ? "pass" : "hold" },
    { label: "覆盖全部预测", value: "not complete", state: "hold" },
    { label: "confidence ≥ 0.60", value: confidence.toFixed(2), state: confidence >= 0.6 ? "pass" : "hold" },
  ];
}

function buildFrameworkLayers() {
  return [
    { index: "01", name: "API / SSE", path: "apps/api/src/routes/runs.ts", detail: "POST /runs · stream · resume" },
    { index: "02", name: "WORKFLOW", path: "packages/agents/.../workflow.ts", detail: "runtime · context · persistence" },
    { index: "03", name: "LANGGRAPH", path: "scientific-graph.ts", detail: "A / B / C / D · checkpointed State" },
    { index: "04", name: "TOOLS / MCP", path: "packages/tools/src + packages/mcp/src", detail: "FITS · solar data · search · sandbox" },
    { index: "05", name: "STORAGE / ARTIFACTS", path: "packages/storage/src/repo", detail: "records · snapshots · runs · checksums" },
  ];
}
...
function buildGraphNodes() {
  return [
    ...
    { id: "A.verify", title: "结构检查", io: "schema · predictions · falsification", color: "var(--solar)" },
    { id: "B.run", title: "证据工作组", io: "agents → evidence / tasks", color: "var(--cyan)" },
    { id: "BC.verify", title: "证据晋级", io: "provenance → auditable records", color: "var(--warn)" },
...
async function loadDemoData() {
  ...
  const maxConfidence = Math.max(...(result.hypotheses ?? []).map((item) => Number(item.confidence) || 0), 0);
  ...
  return {
    ...
    modelRoleCount: unique((result.agentExecutions ?? []).filter((item) => String(item.agentId).includes("model")).map((item) => item.agentId)).length,
