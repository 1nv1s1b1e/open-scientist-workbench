...
function TaskCard({
  ...
  const Icon = mode === 'executed' || mode === 'completed-later' ? CheckCircle2 : mode === 'external' ? ExternalLink : Beaker
  const status =
    mode === 'executed'
  ...
  return (
    ...
    >
      <span className="task-status-icon"><Icon className="h-3.5 w-3.5" /></span>
      <div className="min-w-0 flex-1">
        ...
        <p>{task.objective}</p>
        <dl>
          <div><dt>执行方式</dt><dd>{executorLabel(task.executorId)}</dd></div>
          <div><dt>区分结果</dt><dd>{task.discriminatingOutcomes?.length ?? 0} 项判据</dd></div>
          <div><dt>所需来源</dt><dd>{task.requiredSourceIds?.length ?? 0} 个</dd></div>
          <div><dt>路由</dt><dd>{task.route === 'B' ? '补充证据' : '返回假设/外部能力'}</dd></div>
        </dl>
      </div>
    </motion.article>
  )
  ...
  return (
    <section className="workbench-panel task-queue-panel">
      ...
      <div className="task-route-explainer">
        <span><CheckCircle2 className="h-3.5 w-3.5" /> 已执行任务必须有结果证据</span>
        <ArrowDown className="h-3.5 w-3.5" />
        <span><Clock3 className="h-3.5 w-3.5" /> 新任务按执行器能力进入下一轮或外部队列</span>
      </div>
      <div className="task-queue-lanes">
        <TaskLane title="本轮实际执行" description="这些任务在本轮调用了注册执行器，并写回结果证据。" tasks={executed} evidence={evidence} mode="executed" />
        <TaskLane title="本轮提出，后续已完成" description="任务在本轮形成，但结果来自后续轮次；时间边界不会混写。" tasks={completedLater} evidence={evidence} mode="completed-later" />
        <TaskLane title="系统内可执行" description="已有注册执行器，进入下一轮后可以自动处理。" tasks={runnable} evidence={evidence} mode="runnable" />
        <TaskLane title="需要外部能力" description="当前缺少数据、算法或人工步骤，保持计划状态，不伪装为已完成。" tasks={external} evidence={evidence} mode="external" />
